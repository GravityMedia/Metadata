...
class Genres
{
    ...
    /**
     * Returns the name of the genre at $index of known genres. If $index is out of range a null string will be
     * returned.
     *
     * @param int $index
     *
    ...
    /**
     * Get the genre index for the (case sensitive) genre name. If the genre is not known 255 (which signifies an
     * unknown genre) will be returned.
     *
     * @param string $name
     *
     * @return int
    ...
    {
        $index = array_search($name, self::$genres);
        if (false === $index) {
            return 255;
        }
